...
pub fn parse_mft_stream(record: &mut [u8], bytes_per_sector: usize) -> Result<MftStream> {
    ...
    let first_attr_offset = read_u16(record, 20)
        .context("invalid first attribute offset")? as usize;
    ...
    while offset + 16 <= record.len() {
        ...
        let attr_len = read_u32(record, offset + 4)
            .context("invalid attribute length")? as usize;

        if attr_len < 16 || offset + attr_len > record.len() {
            bail!("corrupted MFT attribute");
        }

        let attr = &record[offset..offset + attr_len];

        if attr_type == ATTR_TYPE_DATA
            && attr.get(8).copied() == Some(1)
            && attr.get(9).copied() == Some(0)
        {
            let mapping_offset = read_u16(attr, 32)
                .context("invalid mapping pairs offset")? as usize;
            ...
            let mapping_pairs = attr
                .get(mapping_offset..)
                .context("mapping pairs out of bounds")?;
            let runs = parse_mapping_pairs(mapping_pairs)?;

            if runs.is_empty() {
                bail!("MFT DATA attribute has no data runs");
            }
...
pub fn parse_user_file_record(
    record_number: u64,
    record: &mut [u8],
    bytes_per_sector: usize,
) -> Option<ParsedRecord> {
    if record.len() < 48 || record.get(0..4) != Some(b"FILE") {
        return None;
    }

    if apply_update_sequence_array(record, bytes_per_sector).is_err() {
        return None;
    }
    ...
    while offset + 16 <= record.len() {
        ...
        match attr_type {
            ATTR_TYPE_FILE_NAME => {
                if let Some(name) = parse_file_name_attribute(attr) {
                    names.push(name);
                }
            }
            ATTR_TYPE_DATA => {
                add_data_attribute_size(attr, &mut size, &mut allocated);
            }
            _ => {}
        }
    ...
    Some(ParsedRecord {
        record_number,
        parent_record: selected_name.parent_record,
        name: selected_name.name.clone(),
        is_dir,
        size,
        allocated,
    })
...
fn apply_update_sequence_array(record: &mut [u8], bytes_per_sector: usize) -> Result<()> {
    if bytes_per_sector < 2 || record.len() < bytes_per_sector {
        bail!("invalid sector size for FILE record");
    }

    if record.get(0..4) != Some(b"FILE") {
        bail!("not an NTFS FILE record");
    }
    ...
    for sector_index in 0..sector_count {
        ...
        if actual != update_sequence_number {
            bail!("FILE record update sequence mismatch");
        }

        let fixup_offset = usa_offset + 2 * (sector_index + 1);
        let fixup = read_u16(record, fixup_offset)
            .context("invalid fixup")?
            .to_le_bytes();
...
fn add_data_attribute_size(attr: &[u8], size: &mut u64, allocated: &mut u64) {
    ...
    if !non_resident {
        if let Some(value_len) = read_u32(attr, 16) {
            *size = size.saturating_add(value_len as u64);
        }
        return;
    }
    ...
    let physical_size =
        if attr_flags & (ATTR_FLAG_COMPRESSED | ATTR_FLAG_SPARSE) != 0 && compressed_size > 0 {
            compressed_size
        } else {
            allocated_size
        };
...
fn choose_name(names: &[NameCandidate]) -> Option<&NameCandidate> {
    names.iter().min_by_key(|candidate| {
        (
            namespace_rank(candidate.namespace),
            candidate.name.len(),
