...
use crate::scanner::ntfs::{boot::parse_boot_sector, record::{MftStream, ParsedRecord}};
use crate::scanner::tree::{finalize_tree, TreeNode};

#[cfg(windows)]
use std::{fs::{File, OpenOptions}, os::windows::fs::{FileExt, OpenOptionsExt}, time::Instant};

#[cfg(windows)]
use windows_sys::Win32::Storage::FileSystem::{
    FILE_FLAG_BACKUP_SEMANTICS, FILE_FLAG_SEQUENTIAL_SCAN, FILE_SHARE_DELETE, FILE_SHARE_READ, FILE_SHARE_WRITE,
};
...
#[cfg(windows)]
pub fn scan_volume(ctx: &ScanContext, root: &str) -> Result<ScanResult> {
    let started = Instant::now();
    let root_display = drive_root_from_input(root)?;
    let device_path = volume_device_path(&root_display)?;

    ctx.progress.emit("ntfs.open", 0, None, &format!("正在打开卷 {device_path}"));
    ...
    let mft_record0_offset = boot.mft_lcn.checked_mul(boot.cluster_size).context("MFT offset overflow")?;
    let mut mft_record0 = vec![0u8; boot.file_record_size];
    read_exact_at(&volume, mft_record0_offset, &mut mft_record0).context("读取 $MFT record 0 失败")?;
    ...
    ctx.progress.emit("ntfs.mft", 0, Some(record_count),
        &format!("NTFS 几何: cluster={} bytes, FILE record={} bytes, MFT records≈{}", boot.cluster_size, boot.file_record_size, record_count));

    let parsed_records = enumerate_mft_records(ctx, &volume, &boot, &mft, record_count)?;

    ctx.progress.emit("ntfs.tree", parsed_records.len() as u64, Some(parsed_records.len() as u64 + 1),
        &format!("正在重建目录树({}条记录)...", parsed_records.len()));

    let nodes = build_tree_from_records(ctx, &root_display, parsed_records);
...
#[cfg(not(windows))]
pub fn scan_volume(_ctx: &ScanContext, _root: &str) -> Result<ScanResult> {
    bail!("NTFS MFT scanner is only available on Windows");
}

#[cfg(windows)]
fn enumerate_mft_records(ctx: &ScanContext, volume: &File, boot: &boot::NtfsBoot, mft: &MftStream, record_count: u64) -> Result<Vec<ParsedRecord>> {
    let mem_mapped = unsafe { Mmap::map(volume) };
    match mem_mapped {
        Ok(mmap) => enumerate_via_mmap(ctx, &mmap, boot, mft, record_count),
        Err(_) => enumerate_via_chunks(ctx, volume, boot, mft, record_count),
    }
}

#[cfg(windows)]
fn enumerate_via_mmap(ctx: &ScanContext, volume_mmap: &Mmap, boot: &boot::NtfsBoot, mft: &MftStream, record_count: u64) -> Result<Vec<ParsedRecord>> {
    ...
    for run in &mft.runs {
        ...
        if remaining_mft_bytes == 0 { break; }
        ...
        let disk_offset = (run.lcn as u64).checked_mul(boot.cluster_size).context("MFT data run disk offset overflow")? as usize;
        ...
        let parsed_chunk: Vec<Option<ParsedRecord>> = data.par_chunks(rec_size).enumerate()
            .map(|(i, raw)| {
        ...
        for rec in parsed_chunk {
            if let Some(record) = rec { parsed.push(record); }
        }

        if last_emit.elapsed().as_millis() > 220 {
            ctx.progress.emit("ntfs.mft", records_seen, Some(record_count),
...
#[cfg(windows)]
fn enumerate_via_chunks(ctx: &ScanContext, volume: &File, boot: &boot::NtfsBoot, mft: &MftStream, record_count: u64) -> Result<Vec<ParsedRecord>> {
    let mut parsed = Vec::with_capacity(record_count.min(1_000_000) as usize);
    let mut records_seen = 0u64;
    let mut stream_bytes_read = 0u64;
    ...
    for run in &mft.runs {
        ...
        let disk_base = (run.lcn as u64).checked_mul(boot.cluster_size).context("MFT data run disk offset overflow")?;
        ...
        while run_offset < bytes_to_process && records_seen < record_count {
            ...
            if prefix_len > 0 { buf[..prefix_len].copy_from_slice(&pending); }
            read_exact_at(volume, disk_base + run_offset, &mut buf[prefix_len..prefix_len + read_len])?;
            ...
            let parsed_chunk: Vec<Option<ParsedRecord>> = buf[..complete_len].par_chunks_mut(rec_size).enumerate()
                .map(|(i, record_buf)| record::parse_user_file_record(records_seen + i as u64, record_buf, bps))
                .collect();

            records_seen += parsed_chunk.len() as u64;
            for rec in parsed_chunk { if let Some(r) = rec { parsed.push(r); } }
...
fn build_tree_from_records(ctx: &ScanContext, root: &str, records: Vec<ParsedRecord>) -> Vec<crate::models::NodeDto> {
    let filtered: Vec<ParsedRecord> = records.into_iter()
        .filter(|record| {
    ...
    let max_frn = filtered.iter().map(|r| r.record_number).max().unwrap_or(0) as usize;
    let mut frn_to_node_id = vec![u32::MAX; max_frn + 1];

    for record in &filtered {
        let id = nodes.len() as u32;
        frn_to_node_id[record.record_number as usize] = id;
        nodes.push(TreeNode::new(id, record.name.clone(), None, record.is_dir, record.size, record.allocated, Some(record.created), Some(record.modified), Some(record.accessed)));
    }

    for (index, record) in filtered.iter().enumerate() {
        let id = (index + 1) as u32;
        let mut parent_id = if record.parent_record == 5 { 0 }
            else {
        ...
        if parent_id != 0 && !nodes[parent_id as usize].is_dir { parent_id = 0; }

        nodes[id as usize].parent = Some(parent_id);
        nodes[parent_id as usize].children.push(id);
    }

    ctx.progress.emit("ntfs.aggregate", 1, Some(2), "正在聚合目录大小...");
    let result = finalize_tree(ctx, nodes);
    ctx.progress.emit("ntfs.done", 2, Some(2), &format!("树构建完成，共 {} 个节点", result.len()));
    result
}
...
fn open_volume(device_path: &str) -> Result<File> {
    OpenOptions::new()
        ...
        .open(device_path)
        .with_context(|| format!("无法打开卷 {device_path}。NTFS MFT 原始读取通常需要管理员权限，请以管理员身份运行。"))
}
...
fn read_exact_at(file: &File, offset: u64, buf: &mut [u8]) -> std::io::Result<()> {
    ...
    while done < buf.len() {
        ...
        if read == 0 { return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "unexpected EOF")); }
        done += read;
    }
    Ok(())
}
...
fn volume_device_path(root: &str) -> Result<String> {
    if root.len() >= 2 && root.as_bytes()[1] == b':' {
        ...
        Ok(format!(r"\\.\{}:", drive))
    } else { bail!("invalid Windows drive root: {root}"); }
}
