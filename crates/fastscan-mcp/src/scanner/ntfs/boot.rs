...
pub fn parse_boot_sector(buf: &[u8]) -> Result<NtfsBoot> {
    if buf.len() < 512 { bail!("boot sector buffer too small"); }
    if buf.get(3..11) != Some(b"NTFS    ") { bail!("目标卷不是 NTFS 文件系统"); }
    ...
    if bytes_per_sector == 0 || sectors_per_cluster == 0 || cluster_size == 0 { bail!("invalid NTFS cluster geometry"); }
    ...
    if clusters_per_file_record == 0 { bail!("invalid clusters per file record"); }
    let file_record_size = if clusters_per_file_record > 0 {
        cluster_size.checked_mul(clusters_per_file_record as u64).context("file record size overflow")?
    } else {
        let exponent = (-(clusters_per_file_record as i16)) as u32;
        1u64.checked_shl(exponent).context("file record size shift overflow")?
    };
    if !(512..=1024 * 1024).contains(&file_record_size) { bail!("unsupported NTFS FILE record size: {file_record_size}"); }
    Ok(NtfsBoot { bytes_per_sector, sectors_per_cluster, cluster_size, total_sectors, mft_lcn, file_record_size: file_record_size as usize })
}
